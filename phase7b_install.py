"""Install Phase 7B: render and splice approved dynamic scenes into the full demo video."""
from pathlib import Path

ROOT = Path.cwd()

FILES = {
    "app/services/dynamic_video_render_service.py": r'''"""Render approved dynamic scenes and integrate them into the demo video."""
...
''',
    "app/pipeline/step_09b_render_dynamic_video.py": r'''"""Pipeline Step 09B: Render the integrated dynamic demo video."""
from __future__ import annotations
import argparse
import sys
from pathlib import Path
...
if __name__ == "__main__":
    sys.exit(main())
''',
    "tests/test_dynamic_video_render_service.py": r'''"""Tests for Phase 7B media and scene helpers."""
from app.services.dynamic_video_render_service import DynamicVideoRenderService
...
def patch_once(path_value, old, new, label):
    ...
    if old not in text:
        raise RuntimeError(
            f"Patch point not found for {label}: {path_value}"
        )
...
patch_once(
    "run_pipeline.py",
    (
        "from app.pipeline.step_05k_publish_dynamic_components "
        "import publish_dynamic_components\n"
    ),
    (
        "from app.pipeline.step_05k_publish_dynamic_components "
        "import publish_dynamic_components\n"
        "from app.pipeline.step_09b_render_dynamic_video "
        "import render_dynamic_video\n"
    ),
    "Phase 7B import",
)
patch_once(
...
ignore_text = ignore_path.read_text(encoding="utf-8-sig")
rule = "renderer/remotion-project/.phase7_dynamic_render_workspaces/\n"
...
print("Phase 7B installation complete.")
